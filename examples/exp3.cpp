...
struct early_initializer
{
    ...
    {
        std::ios_base::Init z;
        std::cout << "zzz constructor from Data::init() " << __PRETTY_FUNCTION__ << " environ: " << (void *)environ
                  << std::endl;
        std::cout << "zzz constructor from Data::init() " << __PRETTY_FUNCTION__ << " argv: " << (void *)argv
                  << std::endl;
...
struct Data
{
    Data() { std::cout << "Data constructor\n"; }
#if 0
  [[ using gnu : used, constructor ]] static void init() {
    std::ios_base::Init z;
